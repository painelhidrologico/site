...
function renderRiverCalculator(){
 ...
 if(backendFresh){
   ...
   // O nível atual vem sempre da leitura oficial ao vivo.
   // O workflow fornece as projeções, mas não deve congelar o nível
   // exibido quando sua execução estiver alguns minutos atrasada.
   p={...p,current:Number(blumenauRiverLevel)};
   for(const h of ['2','4','6','8','12']){
     p['p'+h]=Number(backendProjection.pred[h]);
     p['components'+h]=backendProjection.components?.[h]||{};
   }
   p.status=projectionStatus(Number(blumenauRiverLevel));
